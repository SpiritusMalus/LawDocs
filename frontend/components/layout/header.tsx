  ...
  return (
    <header className="bg-gray-900 sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-gray-900/95">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <Link
          href="/"
          className="flex items-center gap-2 font-semibold text-lg text-white min-h-11 py-1"
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm text-gray-400">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="hover:text-white transition-colors min-h-11 flex items-center px-1"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          {isAuthenticated ? (
            ...
            <>
              <Link
                href="/login"
                className="hidden md:inline-flex h-11 px-4 items-center gap-2 text-sm font-medium text-gray-400 hover:text-white transition-colors"
              >
