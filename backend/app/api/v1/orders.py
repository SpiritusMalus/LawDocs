...
@router.post("/draft", response_model=DraftOrderResponse, status_code=status.HTTP_201_CREATED)
@limiter.limit("10/minute")
async def create_draft_order_encrypted(
    request: Request,
    ...
    """
    Создаёт черновик заказа с зашифрованными данными формы (E2EE).

    Клиент отправляет:
    - situation_id: ID ситуации
    - encrypted_form_data: зашифрованная форма (TweetNaCl.js на клиенте)
    - consent: согласие на обработку ПДн

    Сервер:
    - Проверяет согласие
    - Сохраняет зашифрованные данные (не может расшифровать без private key)
    - Логирует операцию
    """
    ...
    if not body.consent:
        logger.warning("order_created_no_consent", extra={"action": "order_created", "user_id": str(current_user.id), "ip": ip})
        raise HTTPException(
    ...
    try:
        # Создать заказ с зашифрованными данными
        order = Order(
            user_id=current_user.id,
            situation_id=body.situation_id,
            form_data={"encrypted": body.encrypted_form_data},  # Сохранить как зашифрованное
            status="draft",
        )
        db.add(order)
        await db.flush()

        # Обновить информацию о согласии пользователя
        ...
        await AuditLogger.log_access(
            session=db,
            user_id=current_user.id,
            action="order_created_e2ee",
            data_type="form_data_encrypted",
            ip_address=ip,
            details={
                "order_id": order.id,
                "situation_id": body.situation_id,
                "encrypted_size": len(body.encrypted_form_data),
                "consent": True,
            },
        )
        ...
        return DraftOrderResponse(
            order_id=order.id,
            status="draft",
            message="Заказ создан. Ваши данные защищены сквозным шифрованием.",
        )

    except Exception as e:
        logger.error(
            "order_created_e2ee_failed",
            extra={"action": "order_created_e2ee", "user_id": str(current_user.id)},
            exc_info=True,
        )
        raise HTTPException(
